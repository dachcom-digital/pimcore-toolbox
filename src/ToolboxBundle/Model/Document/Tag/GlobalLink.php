    ...
    public function getHref()
    {
        ...
        if ($this->data['internalType'] == 'object') {
            ...
            if (count($objectInfo) == 2) {
                $path = FALSE;
                $cmdEv = \Pimcore::getEventManager()->trigger('toolbox.url.objectFrontendUrl', NULL, ['className' => $objectInfo[0], 'path' => $objectInfo[1]]);

                if ($cmdEv->stopped()) {
                    $path = $cmdEv->last();
                    if (!empty($path)) {
                        $path = \ToolboxBundle\Tool\GlobalLink::parse($path);
                    }
                }
            ...
            if ($this->data['internalType'] == 'document') {
                if ($doc = Document::getById($this->data['internalId'])) {
                    if (!Document::doHideUnpublished() || $doc->isPublished()) {
                        $path = $doc->$method();
                        $this->data['path'] = \ToolboxBundle\Tool\GlobalLink::parse($path);
                    }
                }
